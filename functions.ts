function square1(num){ //parameter type any avoid it 
    return num*num
}



function square2(num:number){ //parameter type number
    return num*num
}
...
function greet(person:string="stranger"){ //type string  "stranger" for default if you provide no name"
...
function square(num:number):number{ //parameter type number
    num*num
...
function rando(num:number){  //function rando(num: number): string | number
    if(Math.random()<0.5){
...
function prinTwice(msg:string): void{ //function prinTwice(msg: string): void
    console.log(msg)
    console.log(msg)
